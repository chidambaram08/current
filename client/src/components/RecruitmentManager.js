...
import Sidebar from '../pages/AdminDash/Sidebar';
import Header from '../pages/AdminDash/Header';
...
export default function RecruitmentManager() {
    ...
    return (
        <div className='recruitment-manager-container'>
            <Header />
            <div className="d-flex">
                <Sidebar className="employee-sidebar" />
                <div className="employee-content flex-grow-1 p-3 ">
                    <div className="row text-light ">
                        ...
                        {isAdding && (
                            <div className="employee-form-group d-flex gap-2 m-3">
                                <input placeholder="Job Title" onChange={(e) => setTitle(e.target.value)} value={title} className="form-control" type="text" />
                                ...
                                {jobs.map((item) =>
                                    <li className="employee-list-group-item bg-primary d-flex justify-content-between align-items-center my-2" key={item._id}>
                                        <div className="d-flex text-light flex-column me-2 p-2">
                                            {editId === -1 || editId !== item._id ? (
                                                ...
                                                <div className="employee-form-group d-flex flex-4 gap-2">
                                                    <input placeholder="Job Title" onChange={(e) => setEditJob({ ...editJob, title: e.target.value })} value={editJob.title} className="form-control" type="text" />
                                                    <input placeholder="Location" onChange={(e) => setEditJob({ ...editJob, location: e.target.value })} value={editJob.location} className="form-control" type="text" />
                                                    <input placeholder="Description" onChange={(e) => setEditJob({ ...editJob, description: e.target.value })} value={editJob.description} className="form-control" type="text" />
                                                </div>
                                            )}
                                        </div>
                                        <div className="d-flex flex-column p-2">
                                            {editId === -1 || editId !== item._id ? (
                                                <>
                                                    <button className="btn btn-sm btn-info m-1" onClick={() => handleEdit(item)}>Edit</button>
                                                    <button className="btn btn-sm btn-danger m-1" onClick={() => handleDelete(item._id)}>Delete</button>
                                                </>
                                            ) : (
                                                <>
                                                    <button className="btn btn-sm btn-info m-1" onClick={handleUpdate}>Update</button>
                                                    <button className="btn btn-sm btn-danger m-1" onClick={handleEditCancel}>Cancel</button>
                                                </>
                                            )}
                                        </div>
                                    </li>
